...
sc = SparkSession.builder.appName("get_sku_id_in_special_attrs").enableHiveSupport().getOrCreate()
...
# if the item contains the special attr, return it's sku id, else return ''
processed_rdd2 = processed_rdd1.map(map_to_special_attrs).filter(lambda x: x != '')

#储存结果
processed_rdd2.repartition(1).saveAsTextFile(save_path)
